...
def analyze_deadlift(video_path, frame_skip=3, scale=0.4):
    ...
    with mp_pose.Pose(min_detection_confidence=0.5, min_tracking_confidence=0.5) as pose:
        while cap.isOpened():
            ...
            try:
                ...
                # Mid-back = weighted average to approximate spine
                middle_back = 0.66 * shoulder + 0.34 * hip
                upper_back_vector = head_point - middle_back
                lower_back_vector = hip - middle_back
                back_bend_angle = calculate_angle(head_point, middle_back, hip)
                ...
                if rep_in_progress:
                    ...
                    max_curvature = max(max_curvature, 180 - back_bend_angle)
                    min_back_angle = min(min_back_angle, back_bend_angle)
                ...
                if not rep_in_progress:
                    if delta_x > 0.08:
                        ...
                        max_curvature = 180 - back_bend_angle
                        min_back_angle = back_bend_angle
